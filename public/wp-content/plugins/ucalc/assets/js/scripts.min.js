(function (e) {
    ...
    e(function () {
        tinymce.create("tinymce.plugins.ucalc_edit_button", {
            init: function (n, r) {
                function i() {
                ...
                function s() {
                    ...
                    jQuery(t).each(function () {
                        if (jQuery(this).is("[data-fasc-style]")) {
                            ...
                            if (e(t).not(n).length == 0 && e(n).not(t).length == 0) {
                            } else {
                            }
                        }
                    })
                ...
                if (tinymce.majorVersion < 4) {
                    alert("FASC Buttons only work with WordPress 3.9 and above.  Please update your WordPress installation")
                }
                n.addButton("ucalc_edit_button", {
                    icon: "ucalc-ico", tooltip: "UCALC", onclick: function () {
                        n.windowManager.open({
                            url: ucalc_popup,
                            //url: r + "/popup.php?key="+ucalc_key+"&ajaxurl=" + encodeURIComponent(fasc_ajaxurl),
                            width: l,
                            height: f
                        }, {plugin_url: r})
                    }
                ...
                n.on("dblClick", function (e) {
                    if (jQuery(e.target).hasClass("fasc-button")) {
                        ...
                        n.windowManager.open({
                            url: r + "/popup.php?key="+ucalc_key,
                            width: l,
                            height: f
                        }, {plugin_url: r, target: jQuery(e.target)})
                    }
                });
                ...
                return {
                    longname: "Forget About Shortcodes Buttons",
                    author: "Ross Morsali",
                    authorurl: "http://www.designsandcode.com/",
                    infourl: "",
                    version: "1.0.4"
                }
            }
        });
        tinymce.PluginManager.add("ucalc_edit_button", tinymce.plugins.ucalc_edit_button)
    })
})(jQuery)